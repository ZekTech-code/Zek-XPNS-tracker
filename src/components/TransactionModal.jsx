...
export default function TransactionModal({ open, filter, setFilter, transactions, currency, onClose, onDelete, hideBalance }) {
  if (!open) return null;
  ...
  return (
    <div className="modal-overlay" id="modalOverlay" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <div className="modal">
        ...
        <TransactionList transactions={sortNewest(filtered)} currency={currency} onDelete={onDelete} emptyText="Nothing here." full hideBalance={hideBalance} />
      </div>
    </div>
  );
}